...
TEST(diffBackward, Vector5) {
  ...
  EXPECT_EQ(result.rows(),
            input.rows()); // test equality of input and result rows
...
TEST(diffBackward, Matrix52) {
  ...
  EXPECT_EQ(result.cols(),
            input.cols()); // test equality of input and result columns
...
TEST(gradient, Matrix52) {
  ...
  EXPECT_EQ(result.rows(),
            depents.rows()); // test equality of input and result rows
  EXPECT_EQ(result.cols(),
            depents.cols()); // test equality of input and result columns
...
TEST(partialLength, StraightLine) {
  ...
  Eigen::Array<double, 5, 1> result{
      FrenetTransform::partialLength(x, y)}; // partial length result

  EXPECT_EQ(result.rows(), x.rows()); // test equality of input and result rows

  Eigen::Array<double, 5, 1> groundTruth{0.0, 1.0, 2.0, 3.0,
                                         4.0}; // ground truth result
...
TEST(first, IncreasingSequencePositive) {
  Eigen::Array<double, 5, 1> input{-3, -1, 3, 5,
                                   6}; // increasing input sequence
...
TEST(first, IncreasingSequenceNegative) {
  Eigen::Array<double, 5, 1> input{-3, -1, -2, -6,
                                   -2}; // negative input sequence
