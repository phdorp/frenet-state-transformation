...
namespace FrenetTransform
{
    ...
    public:
         ...
         * Dertermines the singed lengths along the path from the path origin to the projections.
         * Determines the signed shortest distances to the query point.
         *
         ...
         * @param posFrenet positions corresponding to velocities in Frenet coordinates.
        ...
        {
            // transformation matrices from Cartesian to Frenet frames at given Frenet frame positions
        ...
        {
            // transformation matrices from Frenet to Cartesian frames at given Frenet frame positions
         ...
         * @param velFrenet velocities corresponding to accelerations in Frenet coordinates.
         * @param posFrenet positions corresponding to accelerations in Frenet coordinates.
        ...
        {
            // transformation matrices to determine Cartesian acceleration induced by Frenet velocities
            ...
            // transformation matrices from Cartesian to Frenet frames at given Frenet frame positions
            const auto velTransformsInv { transformInv(velTransform(posFrenet)) };
            // matrix-vector multiplications of transformations with velocity differences
            return {
                velTransformsInv(0, 0) * accDiff.x() + velTransformsInv(0, 1) * accDiff.y(),
                velTransformsInv(1, 0) * accDiff.x() + velTransformsInv(1, 1) * accDiff.y()
            };
        }
        ...
        Points<NumQueries> accCartes(const Points<NumQueries>& accFrenet, const Points<NumQueries>& velFrenet, const Points<NumQueries>& posFrenet) const
        {
            ...
            // transformation matrices from Frenet to Cartesian frames at given Frenet frame positions
            const auto velTransforms { velTransform(posFrenet) };
            // sum of velocity-induced accelerations and transformed Frenet accelerations
            return {
    ...
    private:
        ...
        /**
         * @brief Transformation matrices from Frenet velocities to Cartesian velocity-induced accelerations.
         *
         * @param velFrenet velocities in Frenet frame.
         * @param posFrenet positions in Frenet frame.
         * @return Eigen::Array<ArrayQueries, 2, 2> transformations at "velFrenet" and "posFrenet".
         */
