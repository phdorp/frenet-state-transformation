...
/**
 * @brief Helper function to determine number of elements in 2-dimensional
 * vector.
 *
...
/**
 * @brief Helper function to to make an Eigen::ArrayXd copy of a standard
 * vector.
 *
...
int main(int argc, char *argv[]) {
  ...
  const Eigen::ArrayXd lengthsCircle{
      Eigen::ArrayXd::LinSpaced(101, 0.0, 2 * M_PI)};
  ...
  auto [posMeshX, posMeshY]{
      matplot::meshgrid(matplot::iota(0.5 - bound, 1, 0.5 + bound),
                        matplot::iota(0.5 - bound, 1, 0.5 + bound))};

  // copy mesh to Cartesian points, remove (0, 0)
  FrenetTransform::Points<Eigen::Dynamic> cartesPoints{
      toArray(ravel(posMeshX)), toArray(ravel(posMeshY))};

  // instantiate cartesian velocities and accelerations
  const FrenetTransform::Points<Eigen::Dynamic> cartesVels{
      Eigen::ArrayXd::Ones(cartesPoints.numPoints()) / 2,
      Eigen::ArrayXd::Ones(cartesPoints.numPoints()) / 2};
  const FrenetTransform::Points<Eigen::Dynamic> cartesAccs{
      3 * Eigen::ArrayXd::Ones(cartesPoints.numPoints()) / 4,
      -3 * Eigen::ArrayXd::Ones(cartesPoints.numPoints()) / 4};

  // generate polyline along circle
  const auto circlePoly{
      std::make_shared<FrenetTransform::Polychain<Eigen::Dynamic>>(
          circlePointsX, circlePointsY)};

  // instantiate transform
  FrenetTransform::Transform<Eigen::Dynamic> transform{circlePoly};

  // transform query points to Frenet frame
  const FrenetTransform::Points<Eigen::Dynamic> frenetPointsTf{
      transform.posFrenet(cartesPoints)};
  const FrenetTransform::Points<Eigen::Dynamic> frenetVelsTf{
      transform.velFrenet(cartesVels, frenetPointsTf)};
  const FrenetTransform::Points<Eigen::Dynamic> frenetAccsTf{
      transform.accFrenet(cartesAccs, frenetVelsTf, frenetPointsTf)};

  // transform query points back to Cartesian frame
  const FrenetTransform::Points<Eigen::Dynamic> cartesPointsTf{
      transform.posCartes(frenetPointsTf)};
  const FrenetTransform::Points<Eigen::Dynamic> cartesVelsTf{
      transform.velCartes(frenetVelsTf, frenetPointsTf)};
  ...
  matplot::quiver(
      ...
      std::vector<double>{cartesVels.y().begin(), cartesVels.y().end()}, 0.0);
  ...
  matplot::quiver(
      ...
      std::vector<double>{cartesAccs.y().begin(), cartesAccs.y().end()}, 0.0);
  matplot::xlim({-bound, 1.5 + bound});
  matplot::ylim({-bound, 1.5 + bound});
  matplot::xlabel("Cartesian x-axis/m");
  matplot::ylabel("Cartesian y-axis/m");
  ...
  matplot::ylim(
      {-std::sqrt(2) * bound + radius, std::sqrt(2) * bound - radius});
