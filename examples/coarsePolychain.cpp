...
int main(int argc, char *argv[]) {
  ...
  const auto circlePoly{
      std::make_shared<FrenetTransform::Polychain<Eigen::Dynamic>>(
          circlePointsX(idcsSub), circlePointsY(idcsSub))};

  // get points along the polychain
  const Eigen::ArrayXd lengthsPoly{
      Eigen::ArrayXd::LinSpaced(500, 0.0, 2 * M_PI * radius)};
  ...
  const FrenetTransform::Points<Eigen::Dynamic> cartesPoints{
      {{5.0, 12.0, -2.5, 0.3}}, {{0.0, 12.0, 3.0, 11.5}}};

  // transform query points to Frenet frame
  const FrenetTransform::Points<Eigen::Dynamic> frenetPoints{
  ...
  FrenetTransform::Points<Eigen::Dynamic> normals{
      circlePoly->normal(frenetPoints.x())};
